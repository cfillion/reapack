...
TEST_CASE("unnamed category", M) {
  ...
  try {
    IndexPtr ri = Index::load(L("unnamed_category"));
    FAIL();
  }
...
TEST_CASE("invalid category tag", M) {
  ...
  IndexPtr ri = Index::load(L("wrong_category_tag"));
...
TEST_CASE("invalid package tag", M) {
  ...
  IndexPtr ri = Index::load(L("wrong_package_tag"));
  REQUIRE(ri->categories().empty());
...
TEST_CASE("null package name", M) {
  ...
  try {
    IndexPtr ri = Index::load(L("unnamed_package"));
    FAIL();
  }
  catch(const reapack_error &e) {
...
TEST_CASE("null package type", M) {
  ...
  try {
    IndexPtr ri = Index::load(L("missing_type"));
  }
...
TEST_CASE("unsupported package type", M) {
  ...
  IndexPtr ri = Index::load(L("unsupported_type"));
}

TEST_CASE("read version author", M) {
  UseRootPath root(RIPATH);

  IndexPtr ri = Index::load(L("author"));
...
TEST_CASE("invalid version tag", M) {
  ...
  IndexPtr ri = Index::load(L("wrong_version_tag"));
  REQUIRE(ri->categories().empty());
}
...
TEST_CASE("null package version", M) {
  ...
  try {
    IndexPtr ri = Index::load(L("missing_version"));
    FAIL();
  }
  catch(const reapack_error &e) {
...
TEST_CASE("null source url", M) {
  ...
  try {
    IndexPtr ri = Index::load(L("missing_source_url"));
    FAIL();
  }
  catch(const reapack_error &e) {
...
TEST_CASE("null source file", M) {
  ...
  IndexPtr ri = Index::load(L("missing_source_file"));
  CHECK(ri->packages().size() == 1);
...
TEST_CASE("default source platform", M) {
  ...
  IndexPtr ri = Index::load(L("missing_platform"));
...
TEST_CASE("version changelog", M) {
  ...
  IndexPtr ri = Index::load(L("changelog"));
  CHECK(ri->packages().size() == 1);
...
TEST_CASE("full index", M) {
  ...
  IndexPtr ri = Index::load(L("valid_index"));
  ...
  REQUIRE(ver->name() == VersionName(L("1.0")));
...
TEST_CASE("read index metadata", M) {
  ...
  IndexPtr ri = Index::load(L("metadata"));
...
TEST_CASE("read package metadata", M) {
  ...
  IndexPtr ri = Index::load(L("pkg_metadata"));
...
TEST_CASE("read source platform", M) {
  ...
  IndexPtr ri = Index::load(L("src_platform"));
...
TEST_CASE("read source type override", M) {
  ...
  IndexPtr ri = Index::load(L("src_type"));
...
TEST_CASE("read package description", M) {
  ...
  IndexPtr ri = Index::load(L("pkg_desc"));
...
TEST_CASE("read multiple sections", M) {
  ...
  IndexPtr ri = Index::load(L("explicit_sections"));
