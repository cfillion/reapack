...
TEST_CASE("invalid package name", M) {
  ...
  SECTION("slash") {
    try {
      Package pack(Package::ScriptType, L("hello/world"), nullptr);
      FAIL();
    }
    catch(const reapack_error &e) {
...
TEST_CASE("package versions are sorted", M) {
  ...
  Package pack(Package::ScriptType, L("a"), &cat);
  CHECK(pack.versions().size() == 0);

  Version *final = new Version(L("1"), &pack);
  final->addSource(new Source({}, L("google.com"), final));

  Version *alpha = new Version(L("0.1"), &pack);
  alpha->addSource(new Source({}, L("google.com"), alpha));
...
TEST_CASE("get latest stable version", M) {
  ...
  Version *alpha = new Version(L("2.0-alpha"), &pack);
  alpha->addSource(new Source({}, L("google.com"), alpha));
  ...
  SECTION("an older stable release is available") {
    Version *final = new Version(L("1.0"), &pack);
    final->addSource(new Source({}, L("google.com"), final));
...
TEST_CASE("pre-release updates", M) {
  ...
  Version *stable1 = new Version(L("0.9"), &pack);
  stable1->addSource(new Source({}, L("google.com"), stable1));
  pack.addVersion(stable1);

  Version *alpha1 = new Version(L("1.0-alpha1"), &pack);
  alpha1->addSource(new Source({}, L("google.com"), alpha1));
  pack.addVersion(alpha1);

  Version *alpha2 = new Version(L("1.0-alpha2"), &pack);
  alpha2->addSource(new Source({}, L("google.com"), alpha2));
  pack.addVersion(alpha2);

  SECTION("pre-release to next pre-release")
    REQUIRE(pack.lastVersion(false, {L("1.0-alpha1")}) == alpha2);

  SECTION("pre-release to latest stable") {
    Version *stable2 = new Version(L("1.0"), &pack);
    stable2->addSource(new Source({}, L("google.com"), stable2));
    pack.addVersion(stable2);

    Version *stable3 = new Version(L("1.1"), &pack);
    stable3->addSource(new Source({}, L("google.com"), stable3));
    pack.addVersion(stable3);

    Version *beta = new Version(L("2.0-beta"), &pack);
    beta->addSource(new Source({}, L("google.com"), beta));
    pack.addVersion(beta);

    REQUIRE(pack.lastVersion(false, {L("1.0-alpha1")}) == stable3);
  }
}

TEST_CASE("drop empty version", M) {
  Package pack(Package::ScriptType, L("a"), nullptr);
  const Version ver(L("1"), &pack);
...
TEST_CASE("add owned version", M) {
  Package pack1(Package::ScriptType, L("a"), nullptr);
  Package pack2(Package::ScriptType, L("a"), nullptr);

  Version *ver = new Version(L("1"), &pack1);
...
TEST_CASE("add duplicate version", M) {
  Index ri(L("r"));
  Category cat(L("c"), &ri);
  Package pack(Package::ScriptType, L("p"), &cat);
...
TEST_CASE("find matching version", M) {
  ...
  REQUIRE(pack.findVersion({L("1")}) == nullptr);
  REQUIRE(pack.findVersion({L("2")}) == nullptr);

  pack.addVersion(ver);

  REQUIRE(pack.findVersion({L("1")}) == ver);
  REQUIRE(pack.findVersion({L("2")}) == nullptr);
}

TEST_CASE("package full name", M) {
  const Index ri(L("Index Name"));
  const Category cat(L("Category Name"), &ri);
  Package pack(Package::ScriptType, L("file.name"), &cat);

  REQUIRE(pack.fullName() == L("Index Name/Category Name/file.name"));

  pack.setDescription(L("Hello World"));
  REQUIRE(pack.fullName() == L("Index Name/Category Name/Hello World"));
}

TEST_CASE("package description", M) {
  Package pack(Package::ScriptType, L("test.lua"), nullptr);
